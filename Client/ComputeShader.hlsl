...
float DownScale1024to4(uint DTidx, uint GTid, float avgLum)
{
	     ...
	     groupSize *= 4, step1 *= 4, step2 *= 4, step3 *= 4)
	{
		if (GTid % groupSize == 0)
		{
			...
			stepAvgLum += DTidx + step1 < 1920 ? SharedPositions[GTid + step1] : avgLum;
			stepAvgLum += DTidx + step2 < 1920 ? SharedPositions[GTid + step2] : avgLum;
			stepAvgLum += DTidx + step3 < 1920 ? SharedPositions[GTid + step3] : avgLum;
			avgLum = stepAvgLum;
...
void DownScale4to1(uint DTidx, uint GTid, uint Gidy, float avgLum)
{
	if (GTid == 0)
	{
		...
		fFinalAvgLum += DTidx + 256 < 1920 ? SharedPositions[GTid + 256] : avgLum;
		fFinalAvgLum += DTidx + 512 < 1920 ? SharedPositions[GTid + 512] : avgLum;
		fFinalAvgLum += DTidx + 768 < 1920 ? SharedPositions[GTid + 768] : avgLum;
                            ...
                            uint3 DTid : SV_DispatchThreadID)
{
	...
	{
		float stepAvgLum = avgLum;
		stepAvgLum += DTid.x + 1 < GroupSize ? SharedAvgFinal[DTid.x + 1] : avgLum;
		stepAvgLum += DTid.x + 2 < GroupSize ? SharedAvgFinal[DTid.x + 2] : avgLum;
		stepAvgLum += DTid.x + 3 < GroupSize ? SharedAvgFinal[DTid.x + 3] : avgLum;
	...
	{
		float stepAvgLum = avgLum;
		stepAvgLum += DTid.x + 4 < GroupSize ? SharedAvgFinal[DTid.x + 4] : avgLum;
		stepAvgLum += DTid.x + 8 < GroupSize ? SharedAvgFinal[DTid.x + 8] : avgLum;
		stepAvgLum += DTid.x + 12 < GroupSize ? SharedAvgFinal[DTid.x + 12] : avgLum;
	...
	{
		float stepAvgLum = avgLum;
		stepAvgLum += DTid.x + 16 < GroupSize ? SharedAvgFinal[DTid.x + 16] : avgLum;
		stepAvgLum += DTid.x + 24 < GroupSize ? SharedAvgFinal[DTid.x + 24] : avgLum;
		stepAvgLum += DTid.x + 48 < GroupSize ? SharedAvgFinal[DTid.x + 48] : avgLum;
	...
	{
		float stepAvgLum = avgLum;
		stepAvgLum += DTid.x + 64 < GroupSize ? SharedAvgFinal[DTid.x + 64] : avgLum;
		stepAvgLum += DTid.x + 128 < GroupSize ? SharedAvgFinal[DTid.x + 128] : avgLum;
		stepAvgLum += DTid.x + 192 < GroupSize ? SharedAvgFinal[DTid.x + 192] : avgLum;
	...
	{
		float fFinalLumValue = avgLum;
		fFinalLumValue += DTid.x + 256 < GroupSize ? SharedAvgFinal[DTid.x + 256] : avgLum;
		fFinalLumValue += DTid.x + 512 < GroupSize ? SharedAvgFinal[DTid.x + 512] : avgLum;
		fFinalLumValue += DTid.x + 768 < GroupSize ? SharedAvgFinal[DTid.x + 768] : avgLum;
