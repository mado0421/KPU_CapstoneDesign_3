...
void AnimationManager::AddAnimClip(const char* fileName, ID3D12Device* pd3dDevice,
...
{
	return m_uomAnimClip.contains(name);
}
...
//==================================================================
void AnimationCalculate::GetFrameIdxAndNormalizedTime(AnimClip* clip, const float fTime, float& OutfNormalizedTime,
                                                      XMINT4& OutIdx)
{
	float time = fTime;
	...
	for (int timeIdx = 0; timeIdx < clip->vecTimes.size(); timeIdx++)
	{
		...
		if (clip->vecTimes[timeIdx] < time && time <= clip->vecTimes[timeIdx + 1])
		{
			...
			OutfNormalizedTime = (time - clip->vecTimes[OutIdx.y]) / (clip->vecTimes[OutIdx.z] - clip->vecTimes[OutIdx.
				y]);
			return;
		}
	}
...
XMVECTOR AnimationCalculate::GetLocalTransform(AnimClip* clip, const int boneIdx, const float fNormalizedTime,
                                               const XMINT4 frameIdx)
{
...
void AnimationCalculate::InterpolateKeyframe(Keyframe k0, Keyframe k1, Keyframe k2, Keyframe k3, float t, Keyframe& out)
{
	...
	out.xmf3Translation = Interpolate(k0.xmf3Translation, k1.xmf3Translation, k2.xmf3Translation, k3.xmf3Translation,
	                                  t);
}

XMFLOAT3 AnimationCalculate::Interpolate(const XMFLOAT3 v0, const XMFLOAT3 v1, const XMFLOAT3 v2, const XMFLOAT3 v3,
                                         float t)
{
	XMVECTOR resultV = XMVectorCatmullRom(XMLoadFloat3(&v0), XMLoadFloat3(&v1), XMLoadFloat3(&v2), XMLoadFloat3(&v3),
	                                      t);
	XMFLOAT3 result;
