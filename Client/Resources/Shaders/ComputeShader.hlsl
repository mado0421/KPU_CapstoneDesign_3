...
[numthreads(256, 1, 1)]void CS_main(uint3 DTid : SV_DispatchThreadID)
{
...
[numthreads(256, 1, 1)]void CS_VerticalBlur(uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    int gBlurRadius = 4;
...
[numthreads(1, 256, 1)]void CS_HorizontalBlur(uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    int gBlurRadius = 4;
...
float DownScale4x4(uint2 curPixel, uint GTid)
{
    ...
    [unroll] for (int i = 0; i < 4; i++) [unroll] for (int j = 0; j < 4; j++) downScaled += gtxtColorMap.Load(nFullResPos, int2(j, i));
...
{
    [unroll] for (uint groupSize = 4, step1 = 1, step2 = 2, step3 = 3; groupSize < 1024; groupSize *= 4, step1 *= 4, step2 *= 4, step3 *= 4)
    {
        if (GTid % groupSize == 0)
        {
            float stepAvgLum = avgLum;
            stepAvgLum += DTidx + step1 < 1920
                              ? SharedPositions[GTid + step1]
                              : avgLum;
            stepAvgLum += DTidx + step2 < 1920
                              ? SharedPositions[GTid + step2]
                              : avgLum;
            stepAvgLum += DTidx + step3 < 1920
                              ? SharedPositions[GTid + step3]
                              : avgLum;
            avgLum                = stepAvgLum;
...
void DownScale4to1(uint DTidx, uint GTid, uint Gidy, float avgLum)
{
    if (GTid == 0)
    {
        ...
        fFinalAvgLum += DTidx + 256 < 1920
                            ? SharedPositions[GTid + 256]
                            : avgLum;
        fFinalAvgLum += DTidx + 512 < 1920
                            ? SharedPositions[GTid + 512]
                            : avgLum;
        fFinalAvgLum += DTidx + 768 < 1920
                            ? SharedPositions[GTid + 768]
                            : avgLum;
...
[numthreads(1024, 1, 1)]void CS_DownScaleFirstPass(uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID)
{
    // Resolution ������ �δ� ������
    // gtxtPostProcessMap[xy]�� �־�� �ϴµ�
    // �� ũ�Ⱑ RES��ŭ�̱� ����.
    // y�� ��¥�� Dispatch�� 2, RES.y, 1 ��ŭ �ҰŶ� ������
...
[numthreads(1024, 1, 1)]void CS_DownScaleSecondPass(uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    // �տ��� ������ 1, 270, 1 �� ����� (Gid 270������ ���� ����)
    float avgLum = 0.0f;
    ...
    if (DTid.x % 4 == 0)
    {
        ...
        stepAvgLum += DTid.x + 1 < GroupSize
                          ? SharedAvgFinal[DTid.x + 1]
                          : avgLum;
        stepAvgLum += DTid.x + 2 < GroupSize
                          ? SharedAvgFinal[DTid.x + 2]
                          : avgLum;
        stepAvgLum += DTid.x + 3 < GroupSize
                          ? SharedAvgFinal[DTid.x + 3]
                          : avgLum;
    ...
    if (DTid.x % 16 == 0)
    {
        ...
        stepAvgLum += DTid.x + 4 < GroupSize
                          ? SharedAvgFinal[DTid.x + 4]
                          : avgLum;
        stepAvgLum += DTid.x + 8 < GroupSize
                          ? SharedAvgFinal[DTid.x + 8]
                          : avgLum;
        stepAvgLum += DTid.x + 12 < GroupSize
                          ? SharedAvgFinal[DTid.x + 12]
                          : avgLum;
    ...
    if (DTid.x % 64 == 0)
    {
        ...
        stepAvgLum += DTid.x + 16 < GroupSize
                          ? SharedAvgFinal[DTid.x + 16]
                          : avgLum;
        stepAvgLum += DTid.x + 24 < GroupSize
                          ? SharedAvgFinal[DTid.x + 24]
                          : avgLum;
        stepAvgLum += DTid.x + 48 < GroupSize
                          ? SharedAvgFinal[DTid.x + 48]
                          : avgLum;
    ...
    if (DTid.x % 64 == 0)
    {
        ...
        stepAvgLum += DTid.x + 64 < GroupSize
                          ? SharedAvgFinal[DTid.x + 64]
                          : avgLum;
        stepAvgLum += DTid.x + 128 < GroupSize
                          ? SharedAvgFinal[DTid.x + 128]
                          : avgLum;
        stepAvgLum += DTid.x + 192 < GroupSize
                          ? SharedAvgFinal[DTid.x + 192]
                          : avgLum;
    ...
    if (DTid.x == 0)
    {
        ...
        fFinalLumValue += DTid.x + 256 < GroupSize
                              ? SharedAvgFinal[DTid.x + 256]
                              : avgLum;
        fFinalLumValue += DTid.x + 512 < GroupSize
                              ? SharedAvgFinal[DTid.x + 512]
                              : avgLum;
        fFinalLumValue += DTid.x + 768 < GroupSize
                              ? SharedAvgFinal[DTid.x + 768]
                              : avgLum;
...
[numthreads(1024,1,1)]void CS_Bloom(uint3 DTid : SV_DispatchThreadID)
{
