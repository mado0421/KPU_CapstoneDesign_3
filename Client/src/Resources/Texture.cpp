...
* Texture
*=======================================================================*/
void TempTexture::SetByDepthBuffer(ID3D12Device* pd3dDevice, UINT width, UINT height, D3D12_CPU_DESCRIPTOR_HANDLE& dsvCpuHandle, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    if (!IsTexTypeIsNone())
    {
    ...
    pd3dDevice->CreateCommittedResource(&d3dHeapProperties, D3D12_HEAP_FLAG_NONE, &d3dResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, &d3dClearValue, __uuidof(ID3D12Resource), (void**)&m_pd3dTexture);
...
void TempTexture::SetByRenderTarget(ID3D12Device* pd3dDevice, UINT width, UINT height, D3D12_CPU_DESCRIPTOR_HANDLE& rtvCpuHandle, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    if (!IsTexTypeIsNone())
    {
    ...
    hr = pd3dDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(&m_pd3dTexture));

    pd3dDevice->CreateRenderTargetView(m_pd3dTexture, nullptr, rtvCpuHandle);
...
void TempTexture::SetByUnorderedAccessTexture(ID3D12Device*                pd3dDevice,
                                              UINT                         width,
                                              UINT                         height,
                                              ...
                                              D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    ...
    hr = pd3dDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_pd3dTexture));
...
void TempTexture::LoadFromFile(const wchar_t* pszFileName, ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    if (!IsTexTypeIsNone())
    {
...
* Texture Manager
*=======================================================================*/
TextureManager::TextureManager() : m_pd3dDsvDescriptorHeap(nullptr), m_pd3dRtvDescriptorHeap(nullptr), m_pd3dUavDescriptorHeap(nullptr) {}
...
void TextureManager::AddDepthBufferTexture(const char* name, ID3D12Device* pd3dDevice, UINT width, UINT height, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    auto temp = new TempTexture();
...
void TextureManager::AddRenderTargetTexture(const char* name, ID3D12Device* pd3dDevice, UINT width, UINT height, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    auto temp = new TempTexture();
...
void TextureManager::AddUnorderedAccessTexture(const char* name, ID3D12Device* pd3dDevice, UINT width, UINT height, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    auto temp = new TempTexture();
    temp->SetByUnorderedAccessTexture(pd3dDevice, width, height, m_d3dUavCPUDescriptorHandle, m_d3dUavGPUDescriptorHandle, srvCpuHandle, srvGpuHandle);

    m_uomTextures[name] = temp;
}

void TextureManager::LoadFromFile(const char* name, ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, D3D12_CPU_DESCRIPTOR_HANDLE& srvCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE& srvGpuHandle)
{
    auto temp = new TempTexture();
    ...
    string ultimateOfPerfactFilePath = fileHead + name;
...
ID3D12Resource* TextureManager::GetTextureResource(const char* name) { return m_uomTextures[name]->GetTextureResource(); }

void TextureManager::UseForShaderResource(const char* name, ID3D12GraphicsCommandList* pd3dCommandList, UINT rootParameterIdx) { pd3dCommandList->SetGraphicsRootDescriptorTable(rootParameterIdx, m_uomTextures[name]->GetSrvGPUHandle()); }
...
D3D12_CPU_DESCRIPTOR_HANDLE TextureManager::GetDsvCPUHandle(const char* name) { return m_uomTextures[name]->GetDsvCPUHandle(); }

D3D12_CPU_DESCRIPTOR_HANDLE TextureManager::GetRtvCPUHandle(const char* name) { return m_uomTextures[name]->GetRtvCPUHandle(); }

D3D12_CPU_DESCRIPTOR_HANDLE TextureManager::GetUavCPUHandle(const char* name) { return m_uomTextures[name]->GetUavCPUHandle(); }

D3D12_GPU_DESCRIPTOR_HANDLE TextureManager::GetUavGPUHandle(const char* name) { return m_uomTextures[name]->GetUavGPUHandle(); }
