...
XMFLOAT4X4* AnimatorComponent::GetFinalResultAnimationTransform()
{
    ...
    for (int i = 0; i < MAX_BONE_NUM; i++) XMStoreFloat4x4(&result[i], XMMatrixTranspose(XMMatrixMultiply(XMLoadFloat4x4(&m_arrToDressInv[i]), XMLoadFloat4x4(&m_arrToWorld[i]))));
...
XMMATRIX AnimatorComponent::GetFinalResultTransform(int boneIdx) { return XMMatrixMultiply(XMLoadFloat4x4(&m_arrToDressInv[boneIdx]), XMLoadFloat4x4(&m_arrToWorld[boneIdx])); }
...
void AnimatorComponent::CalcToWorld()
{
    ...
    {
        if (-1 != m_arrParentIdx[i]) XMStoreFloat4x4(&m_arrToWorld[i], XMMatrixMultiply(XMMatrixMultiply(XMMatrixRotationQuaternion(XMLoadFloat4(&m_arrLocalRotation[i])), XMLoadFloat4x4(&m_arrToParent[i])),
                                                                                        XMLoadFloat4x4(&m_arrToWorld[m_arrParentIdx[i]])));
        else XMStoreFloat4x4(&m_arrToWorld[i], XMMatrixMultiply(XMMatrixRotationQuaternion(XMLoadFloat4(&m_arrLocalRotation[i])), XMLoadFloat4x4(&m_arrToParent[i])));
    }
}

HumanoidAnimatorComponent::HumanoidAnimatorComponent(Object* pObject, const char* strClipNameForBoneHierarchy) : AnimatorComponent(pObject, strClipNameForBoneHierarchy), m_pAimingMask(new BoneMask(BoneMask::PreDefined::eUpperBody))
{
...
void HumanoidAnimatorComponent::Update(float fTimeElapsed)
{
    ...
    memset(l_arrMovementLocalRotation, NULL, sizeof(XMVECTOR) * MAX_BONE_NUM);
    {
        ...
        for (int i = 0; i < lPair.size(); i++)
        {
            ...
            for (int j = 0; j < clip->vecBone.size(); j++) l_arrMovementLocalRotation[j] += AnimationCalculate::GetLocalTransform(clip, j, fNormalizedTime, xmi4FrameIdx) * lPair[i].second;
        }
    }
    ...
    memset(l_arrAimingLocalRotation, NULL, sizeof(XMVECTOR) * MAX_BONE_NUM);
    {
        ...
        for (int i = 0; i < lPair.size(); i++)
        {
            ...
            AnimationCalculate::GetFrameIdxAndNormalizedTime(clip, l_fTime, fNormalizedTime, xmi4FrameIdx);
            for (int j = 0; j < clip->vecBone.size(); j++) l_arrAimingLocalRotation[j] += AnimationCalculate::GetLocalTransform(clip, j, fNormalizedTime, xmi4FrameIdx) * lPair[i].second;
        }
    }
...
TargetBoardAnimatorComponent::TargetBoardAnimatorComponent(Object* pObject, const char* strClipNameForBoneHierarchy) : AnimatorComponent(pObject, strClipNameForBoneHierarchy)
{
...
void TargetBoardAnimatorComponent::Update(float fTimeElapsed)
{
    ...
    memset(l_arrLocalRotation, NULL, sizeof(XMVECTOR) * MAX_BONE_NUM);
    {
        ...
        for (int i = 0; i < lPair.size(); i++)
        {
            ...
            for (int j = 0; j < clip->vecBone.size(); j++) l_arrLocalRotation[j] += AnimationCalculate::GetLocalTransform(clip, j, fNormalizedTime, xmi4FrameIdx) * lPair[i].second;
        }
    }
