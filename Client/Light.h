...
using LightType = enum LIGHTTYPE
{
	LIGHT_NONE,
	LIGHT_POINT = static_cast<UINT>(1),
	LIGHT_SPOT = static_cast<UINT>(2),
	LIGHT_DIRECTIONAL = static_cast<UINT>(3)
};
...
public:
	...
	UINT AddDirectionalLight(LIGHT_DESC desc, ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
	                         D3D12_CPU_DESCRIPTOR_HANDLE& d3dCbvCPUDescHandle,
	                         D3D12_GPU_DESCRIPTOR_HANDLE& d3dCbvGPUDescHandle, UINT nCascade = 3);
	...
	void SetShaderResource(ID3D12GraphicsCommandList* pd3dCommandList, UINT i)
	{
		m_vecLight[i]->SetShaderResource(pd3dCommandList);
	}

	void UpdateDirectionalLightOrthographicLH(XMFLOAT4X4 xmf4x4CameraView, UINT i)
	{
		m_vecLight[i]->UpdateDirectionalLightOrthographicLH(xmf4x4CameraView);
	}

	UINT GetNumLight() { return static_cast<UINT>(m_vecLight.size()); }
